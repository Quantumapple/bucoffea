...
class vbfhinvProcessor(processor.ProcessorABC):
    ...
    def process(self, df):
        ...
        for region, cuts in regions.items():
            ...
            ezfill('hemak4_ht',  ht=df['hemveto_ht'][mask],   weight=region_weights.weight()[mask])
            ezfill('hemak4_pt_had',  pt=df['hemveto_pt_had'][mask],   weight=region_weights.weight()[mask])
            ezfill('hemak4_ht_had',  ht=df['hemveto_ht_had'][mask],   weight=region_weights.weight()[mask])
